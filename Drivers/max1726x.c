...
{
    return lsb * 315 / 2; /* 157.5uV was 78.125uV per bit 625 / 8 */
}
...
static int max1726x_raw_current_to_uamps(struct max1726x_priv *priv, u32 curr)
{
    ...
    res *= 1562500 / (priv->pdata->rsense * 1000); /* change current now */
    return res;
}
                   ...
                   union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CYCLE_COUNT:
        ...
        val->intval = reg;
        //printk ("POWER_SUPPLY_PROP_CYCLE_COUNT= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_MAX:
        ...
        //printk ("POWER_SUPPLY_PROP_VOLTAGE_MAX= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_MIN_DESIGN:
        ...
        //printk ("POWER_SUPPLY_PROP_VOLTAGE_MIN_DESIGN= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_NOW:
        ...
        //val->intval = max1726x_lsb_to_uvolts(priv, reg) * 200; //M2
        //val->intval = (int)val->intval/100;
        //printk ("POWER_SUPPLY_PROP_VOLTAGE_NOW= %d\n", val->intval);
        pdata->max_battery_voltage_now = val->intval;
        return pdata->max_battery_voltage_now;
        break;
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_AVG:
        ...
        if (ret < 0)
            return ret;
        val->intval = max1726x_lsb_to_uvolts(priv, reg);
        //printk ("POWER_SUPPLY_PROP_VOLTAGE_AVG= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_OCV:
        ...
        val->intval = max1726x_lsb_to_uvolts(priv, reg);
        //printk ("POWER_SUPPLY_PROP_VOLTAGE_OCV= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_CAPACITY:
        ...
        //printk ("POWER_SUPPLY_PROP_CAPACITY= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_CHARGE_FULL:
        ...
        val->intval = (reg * 680) >> 1; /* Capacité batterie - FullCAPRep LSB: 0.5 mAh // modification M2 was *1000 & 680 before but value was wrong*/
        #if DISKIO_PI_BOARD_1_0
            pdata->max_battery_capacity = val->intval; //M2 set the max capacity in the data struct   
        #endif
        //printk ("POWER_SUPPLY_PROP_CHARGE_FULL= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_CHARGE_COUNTER:
        ...
        //printk ("POWER_SUPPLY_PROP_CHARGE_COUNTER= %d\n", val->intval);
        break;
        
    case POWER_SUPPLY_PROP_CHARGE_NOW: // valeur charge now et %
        ret = regmap_read(regmap, MAX1726X_REPCAP_REG, &reg);
        if (ret < 0)
            return ret;
        //printk ("POWER_SUPPLY_PROP_PRESENT= %d\n", pdata->max_battery_prop_present);
        //printk ("tension actuelle (mV)= %d\n", pdata->max_battery_voltage_now / 100);
        //printk ("current_now= %d\n", pdata->current_now);
        
        /* Détection si power supply branché */
        if (pdata->current_now < 0)
            pdata->max_battery_prop_present = 1;
        else
            pdata->max_battery_prop_present = 0;
        
        //printk ("MAX1726X_STATUS_POR= %d\n", (MAX1726X_STATUS_POR)); // 2
        pdata->max_battery_percentage = ((pdata->max_battery_voltage_now - 7150000) / 8400);
        //printk ("pourcentage(tension)= %d\n", pdata->max_battery_percentage);
        //printk ("reg= %d\n", reg)); /* Abandon du reg au profit de Vnow - reg augmente et diminue plusieurs fois durant le cycle */
        //val->intval = (reg * 680) >> 1;
        val->intval = (pdata->max_battery_percentage * 70000) >> 1;        
        #if DISKIO_PI_BOARD_1_0
            val->intval = pdata->max_battery_capacity - val->intval; //M2 charge full - partiel discharge = partial charge
        #endif
        msleep(300); //was 50
        break;
    ...
    case POWER_SUPPLY_PROP_HEALTH:
        ...
        if (ret < 0)
            return ret;
        //printk ("POWER_SUPPLY_PROP_HEALTH= %d\n", val->intval);
        break;
    ...
    case POWER_SUPPLY_PROP_CURRENT_NOW:
        ...
        pdata->current_now = max1726x_raw_current_to_uamps(priv, reg);
        //return pdata->current_now;
        
        //printk ("POWER_SUPPLY_PROP_CURRENT_NOW= %d\n", val->intval);        
        break;
    ...
    case POWER_SUPPLY_PROP_CURRENT_AVG:
        ...
        val->intval = max1726x_raw_current_to_uamps(priv, reg);
        //printk ("POWER_SUPPLY_PROP_CURRENT_AVG= %d\n", val->intval);        
        break;
    ...
    case POWER_SUPPLY_PROP_TIME_TO_EMPTY_AVG:
        ...
        //printk ("POWER_SUPPLY_PROP_TIME_TO_EMPTY_AVG= %d\n", val->intval);        
        break;
    ...
    case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
        ...
        val->intval = (reg * 45) >> 3; /* TTE LSB: 5.625 sec */
        //printk ("POWER_SUPPLY_PROP_TIME_TO_FULL_NOW= %d\n", val->intval);        
        break;
    default:
        return -EINVAL;
    }
...
static void max1726x_set_alert_thresholds(struct max1726x_priv *priv)
{
    ...
    val |= ((pdata->volt_max / 10) << 8); // was pdata->volt_max / 20
...
static int max1726x_init(struct max1726x_priv *priv)
{
    ...
    //printk ("MAX1726X_STATUS_REG= %d\n", (reg & MAX1726X_STATUS_POR));
    ...
    dev_info(priv->dev,"Model Option: %d\n", pdata->model_option);
...
static struct max1726x_platform_data *max1726x_parse_dt(struct device *dev)
{
    ...
    if (ret)
        pdata->volt_min = 7100; /* mV */ /* Disable alert */ //was 0

    ret = of_property_read_u32(np, "valrt-max", &pdata->volt_max);
    if (ret)
        pdata->volt_max = 8400; /* mV */ /* Disable alert */ // was 5100
                ...
                const struct i2c_device_id *id)
{
    ...
    if (client->irq) {
        ...
        dev_err(priv->dev, "No irq configures\n");
    }
